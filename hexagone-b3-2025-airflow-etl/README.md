...
```bash
docker run --pull=always -d --name airflowlocal -p 8080:8080 -v ${PWD}/dags:/opt/airflow/dags apache/airflow:latest standalone
```
...
```bash
docker exec airflowlocal cat /opt/airflow/simple_auth_manager_passwords.json.generated
```
...
## Forcer l'actualisation des DAGs
...
Forcer l'actualisation avec la commande airflow
...
Télécharger un fichier d'exemple depuis [Wimi](https://ecole-hexagone.wimi.pro/shared/#/file/e15cc2afed3f84f060293dbdf246afa45db7d16a060d6cee91350a39b6e4f040) et créer un job permettant de lire ce fichier.
...
Créer un job permettant de lire un fichier disponible à l'addresse HTTP ci-dessous avec le binaire `wget`.

```bash
http://daa28417-5235-405c-a3b6-00e5333d7dc7.pub.instances.scw.cloud:8081/access.log.1
```
...
A chaque fois qu'une ligne est insérée dans la table principale result_img, déclencher une agrégation qui calcule le temps passé entre le premier résultat et le dernier résultat de la table result_img.
